...
void MainSound(int a) // bgm 재생
{
  ...
  mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE,
                 (DWORD)(LPVOID)&openBgm);
  dwID = openBgm.wDeviceID;
  if (a == 0) // 재생 신호시 재생
    mciSendCommand(dwID, MCI_PLAY, MCI_DGV_PLAY_REPEAT,
                   (DWORD)(LPVOID)&openBgm); // 음악 반복 재생
  else                                       // 정시 신호시 정지
    mciSendCommand(dwID, MCI_PAUSE, MCI_DGV_PLAY_REPEAT,
                   (DWORD)(LPVOID)&openBgm); // 재생 정지
}
...
void playsoundline() // 줄 제거 효과음 재생
{
  ...
  mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE,
                 (DWORD)(LPVOID)&openlinesound);
  dwID1 = openlinesound.wDeviceID;
  mciSendCommand(dwID1, MCI_SEEK, MCI_SEEK_TO_START,
                 (DWORD)(LPVOID)NULL); // 음원 재생 위치를 처음으로 초기화
  mciSendCommand(dwID1, MCI_PLAY, MCI_NOTIFY,
                 (DWORD)(LPVOID)&openlinesound); // 음악을 한 번 재생
}
...
void playsoundfall() // 블록 착지 효과음 재생
{
  ...
  mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE,
                 (DWORD)(LPVOID)&openfallsound);
  dwID2 = openfallsound.wDeviceID;
  mciSendCommand(dwID2, MCI_SEEK, MCI_SEEK_TO_START,
                 (DWORD)(LPVOID)NULL); // 음원 재생 위치를 처음으로 초기화
  mciSendCommand(dwID2, MCI_PLAY, MCI_NOTIFY,
                 (DWORD)(LPVOID)&openfallsound); // 음악을 한 번 재생
}
...
int compareblock(int arr[4][4],
                 int a) // 2차원 배열을 블록들의 첫번째 모양과 비교
{
  int i, j;
...
void scrline(void) // 게임의 전체 태두리 출력
{
  ...
  for (i = 0; i < 39; i++) // 두 번째 줄 부터 마지막에서 두 번째 줄 까지는 가장
                           // 왼쪽과 가장 오른쪽에만 그리기
  {
    gotoxy(0, i + 1);
    printf("◈");
...
void topgameline(void) // 상단 경계선
{
  ...
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          currentblock); // 텍스트 색 다시 원래 블록 색으로 지정
  return;
}
...
void blockprint(void) // 대기중인 블록 출력
{
  ...
  y = 7;
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          setcolor(blocksetprint[0]));
  printsq(blocksetprint[0]); // 첫번째 대기

  y = 12;
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          setcolor(blocksetprint[1]));
  printsq(blocksetprint[1]);

  y = 17;
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          setcolor(blocksetprint[2]));
  printsq(blocksetprint[2]);

  y = 22;
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          setcolor(blocksetprint[3]));
  printsq(blocksetprint[3]);

  y = 27;
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          setcolor(blocksetprint[4]));
...
int guidecheckdown(void) // 가이드 블록 최대 하강 좌표 계산
{
  ...
  for (i = 3; i >= 0; i--) {
    for (j = 0; j < 4; j++) {
      if (acblock[i][j] != 0 &&
          gameblock[exy - 7 + 1 + i][(x - 10) / 2 + j] != 0) // 간섭시 1 반환
        return 1;
    }
  }
...
void setgameblock(void) // 블록 고정
{
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      if (acblock[i][j] == 1)
        gameblock[yset() + i][(x - 10) / 2 + j] =
            currentblock; // 현재 블록의 코드값으로 저장
    }
  }
}
...
int blockcheckmoveL(void) // 좌로 이동시 간섭체크
{
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      if (acblock[j][i] != 0 &&
          gameblock[yset() + j][(x - 10) / 2 - 1 + i] != 0) // 간섭시 1 반환
        return 1;
    }
  }
...
int blockcheckdown(void) // 하강시 간섭체크
{
  ...
  for (i = 3; i >= 0; i--) {
    for (j = 0; j < 4; j++) {
      if (acblock[i][j] != 0 &&
          gameblock[yset() + 1 + i][(x - 10) / 2 + j] != 0) // 간섭시 1 반환
        return 1;
    }
  }
...
{
  if (acblock[3][2] != 0 && gameblock[yset() + 3][(x - 10) / 2 + 2] !=
                                0) // I자블록의 (2, 3) 좌표 간섭 체크
  {
    y -= 1;

    if (blockcheckturnU() ==
        3) // 이동한 좌표가 위쪽 간섭 시 이동한 좌표 초기화, 회전 불가 신호
    {
      y += 1;
      return 1;
    }
      ...
      if (blockcheckturnD() == 4) // 이동한 좌표가 아랫쪽 간섭 시 한 칸 더 이동
      {
        ...
        if (blockcheckturnU() == 3) // 다시 이동한 좌표가 위쪽 간섭시 이동한
                                    // 좌표 초기화, 회전 불가 신호
        {
          y += 2;
          return 1;
  ...
  else if ((acblock[1][0] != 0 && gameblock[yset() + 1][(x - 10) / 2] != 0) &&
           x >= 10) // I자블록의 (0, 1)간섭 체크
  {
    x += 2;

    if (blockcheckturnR() ==
        2) // 이동한 좌표가 오른쪽 간섭시 이동한 좌표 초기화, 회전 불가 신호
    {
      x -= 2;
      return 1;
    }
  ...
  else if (blockcheckturnD() == 4) // 하단 간섭 체크
  {
    if (acblock[3][2] == 1) // I자 블록 간섭시
    {
      ...
      if (blockcheckturnU() == 3 ||
          (acblock[1][2] == 1 && gameblock[yset() + 1][(x - 10) / 2 + 2] ==
                                     1)) // 위쪽, 중앙 간섭 체크, 간섭시 이동한
                                         // 좌표 초기화, 회전 불가 신호
      {
        y += 2;
        return 1;
    ...
    if (blockcheckturnU() != 3) // 이동한 좌표 상단 간섭 안 할 때
    {
      if (blockcheckturnL() == 1) // 좌측 간섭체크, 간섭시 좌표이동
      {
        ...
        if (blockcheckturnD() == 4) // 이동한 좌표 하단 간섭 체크, 간섭시 이동한
                                    // 좌표 초기화, 회전 불가 신호
        {
          x -= 2;
          y += 1;
        ...
        else if (blockcheckturnL() == 1) // 이동한 좌표 좌측 간섭 체크, 간섭시
                                         // 이동한 좌표 초기화, 회전 불가 신호
        {
          x -= 2;
          y += 1;
        ...
        else if (blockcheckturnR() == 2) // 이동한 좌표 우측 간섭 체크, 간섭시
                                         // 이동한 좌표 초기화, 회전 불가 신호
        {
          x -= 2;
          y += 1;
        ...
        else if (blockcheckturnU() == 3) // 이동한 좌표 상단 간섭 체크, 간섭시
                                         // 이동한 좌표 초기화, 회전 불가 신호
        {
          x -= 2;
          y += 1;
        ...
        else if (acblock[1][2] == 1 &&
                 gameblock[yset() + 1][(x - 10) / 2 + 2] ==
                     1) // 이동한 좌표 중앙 간섭 체크, 간섭시 이동한 좌표
                        // 초기화, 회전 불가 신호
        {
          x -= 2;
          y += 1;
  ...
  else if (blockcheckturnL() == 1) // 좌측 간섭 체크 후 간섭시 좌표이동
  {
    if (acblock[1][0] == 1) // I자 블록 간섭일 시 좌표이동
    {
      ...
      if (blockcheckturnR() == 2 ||
          (acblock[1][2] == 1 &&
           gameblock[yset() + 1][(x - 10) / 2 + 2] ==
               1)) // 이동한 좌표 우측 간섭 체크, 중앙 간섭 체크, 간섭시 이동한
                   // 좌표 초기화, 회전 불가 신호
      {
        x -= 4;
        return 1;
    ...
    if (blockcheckturnR() == 2) // 이동한 좌표 우측 간섭 체크, 간섭시 좌표 이동
    {
      ...
      else if (acblock[1][2] != 0 &&
               gameblock[yset() + 1][(x - 10) / 2 + 2] !=
                   0) // 이동한 좌표 중앙 간섭 체크, 간섭시 이동한 좌표 초기화,
                      // 회전 불가 신호
      {
        y += 1;
        return 1;
    ...
    else if (acblock[1][2] != 0 && gameblock[yset() + 1][(x - 10) / 2 + 2] !=
                                       0) // 이동한 좌표 중앙 간섭 체크, 간섭시
                                          // 이동한 좌표 초기화, 회전 불가 신호
    {
      y += 1;
      return 1;
...
{
  if (acblock[1][2] == 1 && acblock[2][1] == 1 && acblock[2][2] == 1 &&
      acblock[1][1] == 1) // 정사각형 블록일시 생략
    return;
...
void finishedgameblock(int a) // 줄이 완성되었을 때
{
  ...
  for (i = a; i > 0; i--) {
    for (j = 1; j < 11; j++) {
      gameblock[i][j] =
          gameblock[i - 1][j]; // 완성된 줄의 윗줄부터 하나씩 아랫줄로 이동
      k++;
    }
...
int start(void) // 스타트 화면
{
  ...
  gotoxy(50, 34);
  printf("회전 : S, D, 방향키 상 / 이동 : 방향키 좌, 우 ,하 / 하드드랍 : "
         "스페이스 바");
  gotoxy(117, 16);
  ...
  while (1) // 게임 종료시 메인화면으로 복귀
  {
    ...
    while (1) {
          ...
          speednum == 0) // 게임 시작 1분째에 속도 변화
      {
        ...
        speednum++;
      } else if (endtime - starttime >= 120000 &&
                 speednum == 1) // 게임 시작 2분째에 속도 변화
      {
        if (num == 1)
          speed -= 10;
